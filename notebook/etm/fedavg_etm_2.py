...
print(f"Experiment: FedAvg ETM with {num_topics} topics, LR: 5e-4")
...
for client_id in hospital_ids:
    ...
    optimizer_fn = torch.optim.Adam(client_model.parameters(), lr=5e-5, weight_decay=1e-6)
    client_optimizers[client_id] = optimizer_fn

client_models, elbo_hist, kld_hist, recon_hist, tc_hist, td_hist, tq_hist = federated_learning(client_models, client_optimizers, rounds=200, epochs=5, enc_drop=enc_drop, beta_=0.01
                                                                                               , lambda_=0.2)
