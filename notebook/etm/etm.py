...
for seed in range(5):
    ...
    logger(f"Experiment: ETM with {num_topics} topics")
    ...
    def train(client_models, client_optimizers, epochs=20, beta_=0.05, lambda_=0.1):
        ...
        for client_id in hospital_ids:
            ...
            for local_epoch in range(epochs):  # Local training epochs (can increase as needed)
                ...
                logger(f"Round {local_epoch + 1}/{epochs} - Client: {client_id} - ELBO: {epoch_elbo:.4f} - Recon likelihood: {epoch_recon_likelihood:.4f} - KLD: {epoch_kld:.4f}, TC: {coherence:.4f}, TD: {diversity:.4f}, TQ: {quality:.4f}")
    ...
    # client_models, elbo_hist, kld_hist, recon_hist, tc_hist, td_hist, tq_hist = train(client_models, client_optimizers, epochs=100, beta_=0.02, lambda_=0.5)
    client_models, elbo_hist, kld_hist, recon_hist, tc_hist, td_hist, tq_hist = train(client_models, client_optimizers, epochs=100, beta_=0.02)
    ...
    logger(f"eICU ETM Topic Coherence: {eicu_coherence}", )
    logger(f"eICU ETM Topic Diversity: {eicu_diversity}")
    logger(f"eICU ETM Topic Quality: {eicu_quality}")
    ...
    logger(f"MIMIC ETM Topic Coherence: {mimic_coherence}")
    logger(f"MIMIC ETM Topic Diversity: {mimic_diversity}")
    logger(f"MIMIC ETM Topic Quality: {mimic_quality}")
    ...
    torch.save(client_models[2001].state_dict(), f'../result/etm_{num_topics}_{seed}_mimic_client_model.pth')
    torch.save(client_models[1001].state_dict(), f'../result/etm_{num_topics}_{seed}_eicu_client_model.pth')

    mimic_train_icds = train_icds[2001].cpu().numpy()
    np.save(f"../result/etm_{num_topics}_{seed}_mimic_train_icds.npy", mimic_train_icds)

    mimic_test_icds = test_icds[2001].cpu().numpy()
    np.save(f"../result/etm_{num_topics}_{seed}_mimic_test_icds.npy", mimic_test_icds)

    mimic_train_readmit_row_ids = train_readmit_row_ids[2001]
    np.save(f"../result/etm_{num_topics}_{seed}_mimic_train_readmit_row_ids.npy", mimic_train_readmit_row_ids)

    mimic_test_readmit_row_ids = test_readmit_row_ids[2001]
    np.save(f"../result/etm_{num_topics}_{seed}_mimic_test_readmit_row_ids.npy", mimic_test_readmit_row_ids)

    mimic_train_label_deaths = train_label_deaths[2001]
    np.save(f"../result/etm_{num_topics}_{seed}_mimic_train_label_deaths.npy", mimic_train_label_deaths)

    mimic_test_label_deaths = test_label_deaths[2001]
    np.save(f"../result/etm_{num_topics}_{seed}_mimic_test_label_deaths.npy", mimic_test_label_deaths)

    eicu_train_icds = train_icds[1001].cpu().numpy()
    np.save(f"../result/etm_{num_topics}_{seed}_eicu_train_icds.npy", eicu_train_icds)

    eicu_test_icds = test_icds[1001].cpu().numpy()
    np.save(f"../result/etm_{num_topics}_{seed}_eicu_test_icds.npy", eicu_test_icds)

    eicu_train_readmit_row_ids = train_readmit_row_ids[1001]
    np.save(f"../result/etm_{num_topics}_{seed}_eicu_train_readmit_row_ids.npy", eicu_train_readmit_row_ids)

    eicu_test_readmit_row_ids = test_readmit_row_ids[1001]
    np.save(f"../result/etm_{num_topics}_{seed}_eicu_test_readmit_row_ids.npy", eicu_test_readmit_row_ids)

    eicu_train_label_deaths = train_label_deaths[1001]
    np.save(f"../result/etm_{num_topics}_{seed}_eicu_train_label_deaths.npy", eicu_train_label_deaths)

    eicu_test_label_deaths = test_label_deaths[1001]
    np.save(f"../result/etm_{num_topics}_{seed}_eicu_test_label_deaths.npy", eicu_test_label_deaths)

print(f"ETM with Topics: {num_topics}")
